// Silk Wyrm Bite - Item Unique OnHit Script
//
...
void main()
{
    ...
    effect eBloodsuck = EffectAbilityDecrease(ABILITY_CONSTITUTION, nDam);
...
// If the target wins, send a short message to it and return.
      ...
      {
         FloatingTextStringOnCreature("You escape the silk wyrm's grapple.", oTarget);
         return;
      }
...
// entangled.
      ...
      {
         FloatingTextStringOnCreature("The silk wyrm tries to grapple you!", oTarget);
      }
      else
      {
         FloatingTextStringOnCreature("The silk wyrm is grappling you!", oTarget);
      }
// Apply the Entangle effect no matter whether the target is immune or not. If
// it is, it will have already received an according message that the silk
// wyrm's entangle attempt failed. Otherwise it simply gets entangled as
// intended.
