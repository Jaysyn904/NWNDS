// Spinewyrm Improved Grab & Constrict - Item Unique OnHit Script
//
...
void main()
{
   ...
   effect eConstrictDmg = EffectDamage(d6(2)+10, DAMAGE_TYPE_PIERCING && DAMAGE_TYPE_BLUDGEONING);
   int nSTR_Target;
   int nSTR_Creature;

// Spinewyrm can't constrict colossal creatures.
...
// Deal 2d6+10 piercing &/or crushing damage.
      ApplyEffectToObject(DURATION_TYPE_INSTANT, eConstrictDmg, oTarget);
...
// If the target wins, send a short message to it and return.
      ...
      {
         FloatingTextStringOnCreature("You escape the spinewyrm's grapple.", oTarget);
         return;
      }
// Check for immunity vs entangle. If the target is immune, tell him that the
// creature TRIED to entangle him. Otherwise tell him that he GOT
// entangled.
      if (GetIsImmune(oTarget, IMMUNITY_TYPE_ENTANGLE, oPC))
      {
         FloatingTextStringOnCreature("The spinewyrm tries to grapple you!", oTarget);
      }
      else
      {
         FloatingTextStringOnCreature("The spinewyrm is constricting you!", oTarget);
      }
