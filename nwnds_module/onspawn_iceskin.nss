...
void main()
{
    ...
    //modify to try fix on atropal aura effects.
    //NC:EDIT
    int nWwight = GetTag(OBJECT_SELF)=="Winterwight" ? TRUE : FALSE;
    if(nWwight)ExecuteScript("ww_aura",OBJECT_SELF);
    int nBBorn = GetTag(OBJECT_SELF)=="BLEAKBORN001" ? TRUE : FALSE;
    if(nBBorn)ExecuteScript("bleakaura",OBJECT_SELF);
