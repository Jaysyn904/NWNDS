...
object oFall=GetNearestObjectByTag ("WP_unearth01");//the object that stores the fall location, set manually by the builder
...
//during a transition between areas.
    if (sDirection == "UP")//If the placeable is a climb UP placeable
        {
        oDestination=GetObjectByTag ("WP_abandonedm02");//get the climb up waypoint
        }
    else if (sDirection == "DOWN")//if the placeable is a climb DOWN placeable
        {
        oDestination=GetObjectByTag ("WP_unearth01");//Get the climb down waypoint
        }
